...
- 🔒 **房间口令与端到端加密**：创建房间时设置口令，加入需匹配；口令非空时自动启用 AES-GCM 加密，中继服务器只能看到密文。
...
- 数据**默认未加密**：旧模式未设置房间口令时，中继服务器能看到明文。创建房间时填写**房间口令**即可自动启用 **AES-GCM 端到端加密**，中继服务器只能看到密文。请只使用自己信任的服务器、只和好友联机。
